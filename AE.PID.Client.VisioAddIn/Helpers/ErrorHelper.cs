...
namespace AE.PID.Client.VisioAddIn
{
    ...
    {
        private const string HighlightShapeWithDuplicatedDesignationScope = "Highlight Shape With Duplicated Designation Within Group";
        ...
        {
            var selection = page.CreateSelection(VisSelectionTypes.visSelTypeByLayer, VisSelectMode.visSelModeSkipSuper, LayerDict.Validation);
        ...
        /// To help user locate the equipment with the wrong designation number, a mask will be placed on the duplicated equipments.
        /// </summary>
        /// <param name="page"></param>
        public static void HighlightShapeWithDuplicatedDesignationWithinGroup(IVPage page)
        {
            ...
            try
            {
                var duplicated = page.Shapes.OfType<Shape>()
                    .Where(x => (x.HasCategory("Equipment") || x.HasCategory("Equipments") || x.HasCategory("Instrument") || x.HasCategory("Instruments")) &&
                                !string.IsNullOrEmpty(x.CellsU[CellDict.FunctionElement].ResultStr[VisUnitCodes.visUnitsString]) &&
                                !string.IsNullOrEmpty(x.CellsU[CellDict.FunctionGroup].ResultStr[VisUnitCodes.visUnitsString]))
                    .Select(x => new
                    {
                        x.ID,
        ...
        public static void RemoveUselessLocalValues(IVPage page)
        {
            ...
            try
            {
                ...
                foreach (var shape in shapes)
                {
                    ...
                    if (shape.SectionExists[(short)VisSectionIndices.visSectionProp, (short)VisExistsFlags.visExistsLocally] == (short)VBABool.True)
                    {
        ...
        /// This problem raised because the previous document update bug. After highlighting these shapes, user should replace them with new master manually.
        /// </summary>
        /// <param name="page"></param>
        ...
        public static void HighlightPipelineWithFormulaError(Page page)
        {
            ...
            try
            {
                var pipelines = page.Shapes.OfType<Shape>()
                    .Where(x => x.Master != null && (x.Master.BaseID == BaseIdDict.Pipe || x.Master.BaseID == BaseIdDict.Signal))
                    .ToList();
                ...
                {
                    var id = pipeline.ID;
                    var beginX = pipeline.CellsSRCN(VisSectionIndices.visSectionObject, VisRowIndices.visRowXForm1D, VisCellIndices.vis1DBeginX).ResultIU;
                    var beginY = pipeline.CellsSRCN(VisSectionIndices.visSectionObject, VisRowIndices.visRowXForm1D, VisCellIndices.vis1DBeginY).ResultIU;

                    // Use FormulaU to get the formula to avoid regional issues
                    var formulaBeginX = pipeline.CellsSRCN(VisSectionIndices.visSectionObject, VisRowIndices.visRowXForm1D, VisCellIndices.vis1DBeginX).FormulaU;
                    var formulaBeginY = pipeline.CellsSRCN(VisSectionIndices.visSectionObject, VisRowIndices.visRowXForm1D, VisCellIndices.vis1DBeginY).FormulaU;
        ...
        private static Layer EnsureValidationLayerExist(IVPage page)
        {
            ...
            var layer = page.Layers.OfType<Layer>().SingleOrDefault(x => x.Name == LayerDict.Validation) ?? page.Layers.Add(LayerDict.Validation);
        ...
        {
            var (left, bottom, right, top) = page.Shapes.ItemFromID[id].BoundingBoxMetric((short)VisBoundingBoxArgs.visBBoxDrawingCoords + (short)VisBoundingBoxArgs.visBBoxExtents);
            var rect = page.DrawRectangleMetric(left - 1, bottom - 1, right + 1, top + 1);
            // set as transparent fill
            rect.CellsSRCN(VisSectionIndices.visSectionObject, VisRowIndices.visRowFill, VisCellIndices.visFillPattern).FormulaU = "9";

            // set layer
            rect.CellsSRCN(VisSectionIndices.visSectionObject, VisRowIndices.visRowLayerMem, VisCellIndices.visLayerMember).FormulaU = $"\"{validationLayer.Index - 1}\"";
        }
    }
}
