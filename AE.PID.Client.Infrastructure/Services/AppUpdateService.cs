...
public class AppUpdateService(IConfigurationService configurationService, IApiFactory<IAppApi> apiFactory)
    : IAppUpdateService, IEnableLogger
{
    private readonly string _folder = Path.Combine(configurationService.RuntimeConfiguration.AppDataFolder,
        "tmp");
    ...
    public async Task<string> DownloadAsync(string downloadUrl)
    {
        try
        {
            ...
            // if the url is wrong, or maybe it is a revert package, clear the download url
            if (response.StatusCode is HttpStatusCode.NotFound or HttpStatusCode.NoContent)

                throw new UrlNotValidException("The response is success but no valid result was returned.");

            var fileName = Path.GetFileName(
                GetFilenameFromContentDisposition(response.Content.Headers.ContentDisposition) ??
                Path.ChangeExtension(Path.GetTempFileName(), "exe"));
            ...
            this.Log().Info($"The installer will be saved at {filePath}...");
    ...
    private static string? GetFilenameFromContentDisposition(ContentDispositionHeaderValue contentDisposition)
    {
        ...
        var parts = contentDisposition.ToString().Split(';');
    ...
    private static ProcessStartInfo BuildProcessInfo(string filePath)
    {
        ...
        return extension switch
        {
            ...
            _ => throw new FileExtensionNotSupportException(extension)
        };
    }
}