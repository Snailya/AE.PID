...
public class ConfigurationService : ReactiveObject
{
    ...
    public ConfigurationService()
    {
        ...
        AppNextTime = configuration.NextTime;
        AppCheckInterval = configuration.CheckInterval;

        // setup library configuration
        LibraryNextTime = configuration.LibraryConfiguration.NextTime;
        LibraryCheckInterval = configuration.LibraryConfiguration.CheckInterval;
        ...
        this.WhenAnyValue(x => x.AppNextTime, x => x.AppCheckInterval,
                x => x.LibraryNextTime, x => x.LibraryCheckInterval
            )
            .Select(_ => Unit.Default)
            .Merge(_libraries.Connect()
                .WhenPropertyChanged(x => x.Version)
                .Select(_ => Unit.Default))
            // switch to background
            .Quiescent(TimeSpan.FromSeconds(5), ThreadPoolScheduler.Instance)
            .Subscribe(_ => Save())
            .DisposeWith(_cleanUp);
    }
    ...
    private void Save()
    {
        lock (_lock)
        {
            try
            {
                ...
                _logger.Info($"Configuration saved successfully at path: '{GetPath()}'");
            }
            catch (Exception ex)
            {
    ...
    public TimeSpan AppCheckInterval
    {
        ...
        set
        {
            ...
            {
                _logger.Warn("The minium check interval is 1 hour.");
