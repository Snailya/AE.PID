    ...
    : ControllerBase
{
    ...
    public IActionResult GetLibraries([FromQuery] bool involvePrerelease = false)
    {
        var libraries = dbContext.Libraries
            ...
            .ThenInclude(x => x.LibraryVersionItems)
            .AsEnumerable()
            .Select(
                x =>
                {
                    ...
                    dto.Items = latestVersion.LibraryVersionItems.Select(i => new LibraryItemDto
                    {
                        Name = i.Name,
                        BaseId = i.BaseId,
                        UniqueId = i.UniqueId
    ...
    public IActionResult Upload([FromForm] UploadLibraryDto dto)
    {
        ...
        library.Versions.Add(new LibraryVersion
        {
            FileName = filePath,
            ReleaseNotes = dto.ReleaseNote,
            Version = currentVersion.ToString(),
            LibraryVersionItems = OpenXmlService.GetItems(filePath).ToList()
        });
    ...
    {
        // update the library version hash
        var snapshot = new RepositorySnapshot
        {
            Versions = dbContext.Libraries.Select(x => x.GetLatestVersion(true)!).ToList()
        };
    ...
    public IActionResult GetVersion([FromRoute] int id, [FromRoute] int versionId)
    {
        var version = dbContext.Libraries.Where(x => x.Id == id)
            ...
            .Include(x => x.LibraryVersionItems)
    ...
    public IActionResult GetVersionItems([FromRoute] int id, [FromRoute] int versionId)
    {
        ...
        return Ok(version.LibraryVersionItems.Select(x => x.ToDetailedLibraryItemDto()).ToList());
    }
    ...
    #region -- Api Version 2.0 --

    [MapToApiVersion(2)]
    [HttpGet("api/v{v:apiVersion}/[controller]/snapshots/latest")]
    public IActionResult GetLibrarySnapshot([FromQuery] bool involvePrerelease = false)
    {
        // todo:
        var record = dbContext.RepositorySnapshots.Include(x => x.Versions)
            .Where(x => involvePrerelease ? true : x.Versions.All(i => i.IsReleased == true))
            .OrderByDescending(x => x.CreatedAt).FirstOrDefault();


        return Ok(record == null
            ? null
            : new LibrarySnapshotDto
            {
                Id = record.Id,
                Versions = record.Versions.Select(x => new LibraryVersionDto
                {
                    Id = x.Id,
                    Name = x.Library.Name,
                    Version = x.Version,
                    DownloadUrl = linkGenerator.GetUriByAction(HttpContext, nameof(DownloadLibraryVersion), null,
                        new { id = x.Library.Id, versionId = x.Id }) ?? string.Empty
                }).ToArray()
            });
    }

    [MapToApiVersion(2)]
    [HttpGet("api/v{v:apiVersion}[controller]/{id:int}/versions/{versionId:int}/download")]
    public IActionResult DownloadLibraryVersion([FromRoute] int id, [FromRoute] int versionId)
    {
        var version = dbContext.Libraries.Include(x => x.Versions).SingleOrDefault(x => x.Id == id)
            ?.Versions.SingleOrDefault(x => x.Id == versionId);
        if (version != null)
            if (System.IO.File.Exists(version.FileName))
                // Return the file as a downloadable response
                return PhysicalFile(Path.Combine(Directory.GetCurrentDirectory(), version.FileName),
                    "application/octet-stream", Path.ChangeExtension(version.Library!.Name, "vssx"), true);
